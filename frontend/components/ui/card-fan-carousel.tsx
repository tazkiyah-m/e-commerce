...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-hidden min-h-[500px]">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-full pt-10">
            ...
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-[32px] shadow-2xl">
                ...
                <div className="absolute bottom-6 left-6 z-30">
                  <p className="text-white font-bold text-xl drop-shadow-md">{card.alt}</p>
                </div>
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer absolute w-56 h-80 md:w-72 md:h-[450px] top-0">{image}</a>
            ) : (
              <div key={index} className="fan-card absolute w-56 h-80 md:w-72 md:h-[450px] top-0">{image}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-8 md:mt-12 z-30 relative top-[350px] md:top-[420px]">
